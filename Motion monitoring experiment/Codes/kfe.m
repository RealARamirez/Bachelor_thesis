...
Data = load('workspace_IMUs_kfe1.mat');
...
save('Kfe_orientation_Spline', '-struct', 'sp');
Results.t = t;
Results.yawgt = gt(:, 1).';
...
coef1 = ridge([0; 0; acc1(1,:).'], A, 10, 0);
coef2 = ridge([0; 0; acc1(2,:).'], A, 250, 0);
coef3 = ridge([0; 0; acc1(3,:).'], A, 2, 0);
...
save('Kfe_pos_Spline', '-struct', 'sp');
save('Kfe_Results', '-struct', 'Results');
figure; hold on;
plot(t, gt(:, 1), 'r');
plot(t, outcome(1, :), 'b--');
legend('Ground truth', 'Approximation')
