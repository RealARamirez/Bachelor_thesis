...
Data = load('workspace_IMUs_haa1.mat');
k = 6; nPBB = 7;
t = Data.t; gyr = Data.gyr;
gt = Data.ang_gt;
k = 6; nPBB = 7;
brks = diff(t)./2;
...
save('Haa_orientation_Spline', '-struct', 'sp');
Results.t = t;
Results.yawgt = gt(:, 1).';
...
coef1 = ridge([0; 0; acc1(1,:).'], A, 500, 0);
coef2 = ridge([0; 0; acc1(2,:).'], A, 250, 0);
coef3 = ridge([0; 0; acc1(3,:).'], A, 500, 0);
...
save('Haa_pos_Spline', '-struct', 'sp');
save('Haa_Results', '-struct', 'Results');
figure; hold on;
plot(t, gt(:, 1), 'r');
plot(t, outcome(1, :), 'b--');
legend('Ground truth', 'Approximation')
