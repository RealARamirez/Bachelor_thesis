...
save('Tumour_4_Spline', '-struct', 'sp');
save('Tumour_4_Results', '-struct', 'Results');
[outx outy outz] = sph2cart(azimuth, elevation, outcome.'); out = [outx+mean(x) outy+mean(y) outz+mean(z)];
ptCloud = pointCloud(out);
figure;
pcshow(Tumor)
title('Tumour 4 pre-operative model')
figure;
pcshow(ptCloud)
title('Tumour 4 Spline outcome')
Representation = stlread('Tumour_4.stl');
