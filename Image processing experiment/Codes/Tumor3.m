...
save('Tumour_3_Spline', '-struct', 'sp');
save('Tumour_3_Results', '-struct', 'Results');
[outx outy outz] = sph2cart(azimuth, elevation, outcome.'); out = [outx+mean(x) outy+mean(y) outz+mean(z)];
ptCloud = pointCloud(out);
figure;
pcshow(Tumor)
title('Tumour 3 pre-operative model')
figure;
pcshow(ptCloud)
title('Tumour 3 Spline outcome')
Representation = stlread('Tumour_3.stl');
