...
save('Tumour_2_Spline', '-struct', 'sp');
save('Tumour_2_Results', '-struct', 'Results');
[outx outy outz] = sph2cart(azimuth, elevation, outcome.'); out = [outx+mean(x) outy+mean(y) outz+mean(z)];
ptCloud = pointCloud(out);
figure;
pcshow(Tumor)
title('Tumour 2 pre-operative model')
figure;
pcshow(ptCloud)
title('Tumour 2 Spline outcome')
Representation = stlread('Tumour_2.stl');
